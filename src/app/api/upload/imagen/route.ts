...
export async function POST(request: Request) {
  try {
    ...
    if (!fileExtension || !ALLOWED_IMAGE_FORMATS.includes(fileExtension)) {
      return NextResponse.json(
        {
          error: `Formato no permitido. Formatos permitidos: ${ALLOWED_IMAGE_FORMATS.join(', ')}`,
        },
        { status: 400 }
      )
    }
    ...
    if (fileSizeMB > MAX_IMAGE_SIZE_MB) {
      return NextResponse.json(
        {
          error: `El archivo es demasiado grande. Tamaño máximo: ${MAX_IMAGE_SIZE_MB}MB`,
        },
        { status: 400 }
      )
    }
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from(STORAGE_BUCKETS.IMAGES).getPublicUrl(fileName)
    ...
    console.error('Error en POST /api/upload/imagen:', error)
    return NextResponse.json(
